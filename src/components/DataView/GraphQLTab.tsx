...
import { Space, Button, Spin, Select, message } from 'antd';
  ...
  return (
    <div style={{ height: 'calc(100vh - 140px)', display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div ref={containerRef} style={{ flex: 1, display: 'flex', minHeight: 0, userSelect: 'none' }}>
        <div style={{ width: `${leftWidth}%`, border: '1px solid #e5e8f0', borderRadius: 8, overflow: 'hidden' }}>
        ...
        <div onMouseDown={onMouseDown} style={{ width: 6, cursor: 'col-resize', background: 'transparent', flexShrink: 0, position: 'relative', zIndex: 10 }}>
          <div style={{ position: 'absolute', left: 2, top: 0, bottom: 0, width: 2, background: dragging.current ? '#1677ff' : '#e8e8e8', transition: dragging.current ? 'none' : 'background 0.2s' }} />
        </div>
        <div style={{ flex: 1, overflow: 'auto', background: '#fff', borderRadius: 6, border: '1px solid #f0f0f0' }}>
          <Spin spinning={loading}>
            {error ? (
              ...
              <div style={{ color: '#9aa3b5', padding: '40px 0', textAlign: 'center' }}>{t('graphqlPlaceholder')}</div>
            )}
          </Spin>
        </div>
      </div>
